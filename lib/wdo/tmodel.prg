...
METHOD Open() CLASS TModel
	...
	::oDb1 := WDO():Dbf( cDbf, cCdx )
		   

RETU NIL

METHOD OpenDbf( ) CLASS TModel

	LOCAL cPath 	:= AP_GETENV( 'DOCUMENT_ROOT' ) + AP_GETENV( 'PATH_APP' ) + '/data/'
	LOCAL cFile 	:= cPath + ::cTable
	LOCAL bError   := Errorblock({ |o| ErrorHandlerModel(o) })
	LOCAL cError 	:= ''
	LOCAL oError
...
METHOD Focus( cFocus ) CLASS TModel
...
*-----------------------------------
STATIC FUNCTION ErrorHandlerModel(oError)
*-----------------------------------
